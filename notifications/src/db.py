...
The API (two gunicorn workers, threaded) and the worker process share one
database file, so this module does the three things the previous
implementation did not: WAL journalling, a busy timeout, and one connection
per thread instead of a single module-level connection handed out to every
request.
